...
> ⚠️ **Позиция корзины, снимаемая с продажи, откладывает списание.** Если каталожная позиция подписки перешла в статус `deleting`, очередное списание не проваливается, а переносится на следующий прогон: попытки не сгорают и подписка не уходит в льготный период из-за временного состояния. Верните позицию обычным `POST /catalog` — см. [Каталог → Статусы товара](catalog.md#статусы-товара).
