...
1. **Фича включена.** Фискальные чеки — за kill-switch на стороне ApiPay. Если выключены, любой запрос вернёт `403 fiscal_receipts_disabled`.
...
| `receipt_dispatch_error` | — (в `failed`) | Технический сбой отправки | Повторите с **новым** `client_operation_id` |
