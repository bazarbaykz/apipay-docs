...
ApiPay отправляет 11 типов событий:
...
#### QR-счёт заменён новым (status: cancelled)
...
| `invoice.error_message` | string \| null | Человекочитаемая причина. При `status=error` присутствует всегда; при `status=cancelled` — только если заполнена (например «Заменён новым QR-счётом #N»). В статусах `paid`/`pending`/`expired` отсутствует. |
...
| `invoice.status_changed` | `cancelled` | Счёт отменён: вами через API, кассиром, либо автоматически — при создании нового QR той же кассой (`error_message`: «Заменён новым QR-счётом #N»). |
| `invoice.status_changed` | `expired` | Счёт истёк: 24 часа в Kaspi, либо ~5 минут для QR-токена. |
