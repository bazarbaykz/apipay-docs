...
- Отмена/возврат не поддерживаются — если оплата не пришла, счёт через несколько минут переходит в `expired` (по терминалу от Kaspi).
- Per-org rate limit: **60 QR-запросов в минуту на организацию** (отдельно от общего лимита API).

> ℹ️ **QR-счета сосуществуют.** Создание нового QR на той же кассе **не отменяет** прежние — старый QR остаётся в статусе `pending` и мониторится до своего терминала. Реагируйте на `paid`/`cancelled`/`expired` по каждому `invoice.id` отдельно (при оплате нескольких QR придёт несколько `paid`). Вебхука-замены (`cancelled` с текстом «Заменён новым QR-счётом») больше не существует. Счета по номеру телефона живут 24 часа в Kaspi.
...
5. Отмена/возврат с QR не поддерживается — для отмены просто дождитесь истечения (несколько минут).
...
|--------|----------|----------------|---------------|
| `processing` | Выставляется (отправлен в Kaspi, ожидает подтверждения) | Да | Нет |
