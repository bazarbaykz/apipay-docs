...
> ⚠️ **Только 1 активный QR на организацию.** Создание нового QR-счёта мгновенно делает предыдущий `qr_token_url` невалидным — старый QR на экране кассы перестанет приниматься Kaspi. Сам предыдущий счёт остаётся в статусе `pending` до истечения TTL или оплаты нового. Если касса уже показывает QR — не создавайте следующий, пока не получите webhook по предыдущему или не истечёт TTL (5 минут).
