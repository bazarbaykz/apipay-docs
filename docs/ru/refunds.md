...
### Почему возврат не удался

У возврата со статусом `failed` причина приходит кодом в поле `error_code` — в `GET /invoices/{id}/refunds` и в вебхуке `invoice.refunded` (`refund.error_code`):

| Код | Что значит | Что делать |
|-----|------------|------------|
| `refund_window_expired` | Kaspi отклонил возврат: возможно, истёк срок возврата либо возврат уже сделан | Не повторяйте |
| `refund_rejected_by_kaspi` | Kaspi отклонил возврат по этой операции. Отказ может быть временным | Повторите позже; если не выйдет — выполните возврат вручную в приложении Kaspi Pay |
| `refund_requires_buyer_confirmation` | По этой операции нужен QR-возврат: покупатель подтверждает его сам | Создайте QR-возврат через `POST /qr-refunds` и покажите QR покупателю |

> ⛔ Сервер не повторяет неудавшийся возврат сам: строка со статусом `failed` сразу финальная. Повтор — это **новый запрос возврата** с вашей стороны.

Полный список кодов — [Коды ошибок](errors.md).
