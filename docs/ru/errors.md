...
| `refund_rejected_by_kaspi` | — | async (`invoice.refunded`, `refund.status=failed`) | Kaspi отклонил возврат по этой операции. Отказ может быть временным: имеет смысл повторить позже, а если не выйдет — выполнить возврат вручную в приложении Kaspi Pay. ⛔ Сервер сам этот возврат не повторяет: строка сразу финальная, повтор — это новый запрос возврата с вашей стороны. |
| `refund_requires_buyer_confirmation` | — | async (`invoice.refunded`, `refund.status=failed`) | По этой операции нужен QR-возврат: создайте его через `POST /qr-refunds` и покажите QR покупателю. |
| `qr_refund_execution_attempts_exhausted` | 409 | sync | `POST /qr-refunds/{id}/execute`: число денежных попыток по одной сессии возврата исчерпано. Kaspi при этом не вызывается вовсе — деньги не двигались, сессия остаётся в `customer_identified`. Повтор на этой сессии не поможет: создайте новую сессию возврата, с новым подтверждением покупателя. |
...
| `kyc_daily_limit_reached` | 429 | sync | Молодая организация: пока анкета о бизнесе не одобрена, боевые счета не выставляются вовсе — в `meta.limit` приходит `0`, в `meta.reset_at` — когда лимит сбросится. Песочница не затронута: интеграцию можно полностью отладить там. Заполните короткую анкету в кабинете (`/business-profile`), одобрение обычно занимает несколько часов — после него боевая ось открывается автоматически. |
...
| `kyc_required` | 403 | sync | Подключить кассира можно после одобрения анкеты «Расскажите о бизнесе». Приходит на `POST /connections/{id}/auth/init` и `POST /connections/{id}/auth/send-phone` — SMS кассиру при этом не отправляется вовсе; редко на `POST /connections/{id}/auth/verify-otp`, если одобрение перестало действовать между шагами: код Kaspi уже принят, но подключение не создаётся — сессия закрыта, после одобрения начните с нового `init`. Повторять до одобрения бесполезно: решение принимает человек. Переподключение уже привязанного кассира (тот же номер) этим кодом не отбивается. |
...
| `organization_identity_conflict` | 409 | sync | Подключаемая организация Kaspi не совпадает с той, что закреплена за этой организацией, либо эта пара уже закреплена за другой. Подключение кассира само по себе владельца не переносит. Ответ намеренно не раскрывает реквизиты чужой организации. Если владелец бизнеса действительно сменился, напишите в поддержку 77003076512. |
| `organization_identity_unavailable` | 502 | sync | Kaspi не вернул достоверные данные об организации. Привязка при этом не блокируется, и ранее подтверждённая пара сохраняется: попытка входа просто завершается, повторяйте с нового запроса кода. |
| `connection_identity_unverified` | 422 | sync | Нельзя назначить основным подключение, которое не подтвердило личность организации либо заблокировано. ⚠️ В теле этого отказа приходят только `error` и `message` — поля `error_code` нет. |
...
| `catalog_item_not_found` | 422 / — | sync: `422` на одиночных счетах, запись в `invoices[]` со `status: failed` в `POST /invoices/bulk` | Позиция корзины недоступна: чужая, снятая (`deleted`), стоящая в очереди на снятие (`deleting`) или без цены. Точная причина — текстом в `errors["cart_items.N.catalog_item_id"]`, а в батче — в `message` элемента. Позицию в `deleting` можно вернуть обычным `POST /catalog`, см. [Каталог → Статусы товара](catalog.md#статусы-товара). |
| `catalog_delete_scope_required` | 422 | sync | `POST /catalog/bulk-delete` без списка целей или сразу с двумя списками. Режим теперь один: ровно один `ids[]` **либо** `external_refs[]`. |
...
| `catalog_bulk_delete_mismatch` | 409 | sync | Необязательный `expected_count` разошёлся с фактом (в теле `actual_count`): множество изменилось между `dry_run` и командой. Ничего не удалено — повторите `dry_run`. |
| `catalog_multi_tradepoint` | 409 / — | sync на `POST /catalog/bulk-delete`; async на позиции при `DELETE /catalog/{id}` | У организации несколько торговых точек. Массовое удаление отбивается синхронно (`409`, состояние не меняется). Одиночное удаление принимается с `202`, а код появляется на самой позиции в `error_code`. Обратитесь в поддержку. |
| `catalog_batch_filter_removed` | 422 | sync | Параметр `batch_id` удалён вместе с агрегатом партий и отклоняется явно — у `GET /catalog` и `GET /catalog/errors`. Отбивается по самому наличию параметра: пустой `?batch_id=` тоже даёт `422`. Отбирайте свои позиции точечным `GET /catalog?external_refs[]=` либо окном `from`/`to`. |
...
| `idempotency_conflict` | 409 | sync | Ключ `external_order_id_idempotency` уже использован для платёжной ссылки этой организации (`POST /invoices/qr` с `static: true`). Новая ссылка не создаётся, а в теле приходят `static_qr_id` и `payment_url` прежней — используйте их. У ссылок своё пространство ключей, отдельное от прямых счетов, где дубль отвечает `duplicate_idempotency_key`. Не путайте с `idempotency_key_conflict` — это другой код. |
| `idempotency_key_conflict` | 409 | sync | `Idempotency-Key` уже занят другим телом либо другой каталожной операцией — пространство ключей общее у заливки и массового удаления. Возьмите новый ключ; повторять с тем же бесполезно. |
